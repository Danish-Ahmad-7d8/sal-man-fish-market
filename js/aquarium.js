window.addEventListener("salmanDataLoaded", function () {
  ...
  var chips = document.querySelector("#aquariumCategories");
  var sellerSel = document.querySelector("#aquariumSellerFilter");
  var sortSel = document.querySelector("#aquariumSort");
  var grid = document.querySelector("#aquariumProducts");
  if (!chips || !grid) return;

  var cats = ["All"].concat([...new Set(PRODUCTS.filter(function(p){ return p.type === "aquarium"; }).map(function(p){ return p.category; }))]);

  chips.innerHTML = cats.map(function(c){
    return '<button class="chip ' + (c === "All" ? "active" : "") + '" data-cat="' + c + '">' + c + "</button>";
  }).join("");

  PRODUCTS.filter(function(p){ return p.type === "aquarium"; }).forEach(function(p){
  ...
  function render(){
    var arr = PRODUCTS.filter(function(p){
      return p.type === "aquarium" && (category === "All" || p.category === category) && (seller === "all" || p.sellerName === seller);
    });
    ...
    grid.innerHTML = arr.length ? arr.map(productCard).join("") : '<div class="empty-state">No aquarium fish listed yet. Check back soon!</div>';
  }
  render();
});
